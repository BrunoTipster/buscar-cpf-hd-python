...
def search_files(directory):
    ...
    progress_bar["value"] = 0  # Reiniciar a barra de progresso
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            try:
                ...
                # Atualizar o Label de CPFs válidos encontrados
                valid_cpfs_label.config(text=f'Total de CPFs válidos encontrados: {len(total_valid_cpfs)}')
                cpf_list_label.config(text=f'CPFs válidos: {", ".join(total_valid_cpfs)}')
            except Exception as exc:
                logging.error(f'Ocorreu um erro ao processar o arquivo {futures[future]}: {exc}')
            result_text.insert(tk.END, '\n')
            progress_bar["value"] = i + 1
            root.update_idletasks()  # Atualizar a barra de progresso
...
valid_cpfs_label = tk.Label(frame, text="Total de CPFs válidos encontrados: 0")
valid_cpfs_label.grid(row=1, column=1, columnspan=2, padx=5, pady=5, sticky=tk.W)

cpf_list_label = tk.Label(root, text="CPFs válidos: ")
cpf_list_label.pack(padx=10, pady=5, anchor='w')
